...
func TestRemoveNIC(t *testing.T) {
	...
	if e.isAttached() {
		t.Errorf("link endpoint for removed NIC still attached to a network disatcher")
	}
}

func TestRoutesWithDownNIC(t *testing.T) {
	tests := []struct {
		name   string
		downFn func(t *testing.T, s *stack.Stack, nicID tcpip.NICID)
		upFn   func(t *testing.T, s *stack.Stack, nicID tcpip.NICID)
	}{
		{
			name: "Disabled NIC",
			downFn: func(t *testing.T, s *stack.Stack, nicID tcpip.NICID) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if upFn := test.upFn; upFn != nil {
				// Bringing NIC1 up should make routes to odd addresses available again.
				// Routes to even addresses should continue to be unavailable as NIC2 is
				// still down.
				upFn(t, s, nicID1)
...
func TestRouteWritePacketWithDownNIC(t *testing.T) {
	tests := []struct {
		name   string
		downFn func(t *testing.T, s *stack.Stack, nicID tcpip.NICID)
		upFn   func(t *testing.T, s *stack.Stack, nicID tcpip.NICID)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if upFn := test.upFn; upFn != nil {
				...
				// succeed.
				//
				// TODO(b/147015577): Should we instead completely invalidate all Routes
				// that were bound to a NIC that was brought down at some point?
				upFn(t, s, nicID1)
				testSend(t, r1, ep1, buf)
				testFailingSend(t, r2, ep2, buf, tcpip.ErrInvalidEndpointState)
			}
...
// TestLeaveIPv6SolicitedNodeAddrBeforeAddrRemoval tests that remove an IPv6
