...
func (f *defaultFileFD) PRead(ctx context.Context, dst usermem.IOSequence, offset int64, opts vfs.ReadOptions) (int64, error) {
	...
	return readFromHostFD(ctx, f.inode.hostFD.FD(), dst, offset, int(opts.Flags))
}
...
func (f *defaultFileFD) Read(ctx context.Context, dst usermem.IOSequence, opts vfs.ReadOptions) (int64, error) {
	...
	if f.inode.mayBlock {
		...
		reader := fd.NewReadWriter(f.inode.hostFD.FD())
	...
	f.mu.Lock()
	n, err := readFromHostFD(ctx, f.inode.hostFD.FD(), dst, f.offset, int(opts.Flags))
	f.offset += n
	f.mu.Unlock()
	return n, err
}
...
func (f *defaultFileFD) PWrite(ctx context.Context, src usermem.IOSequence, offset int64, opts vfs.WriteOptions) (int64, error) {
	...
	return writeToHostFD(ctx, f.inode.hostFD.FD(), src, offset, int(opts.Flags))
}
...
func (f *defaultFileFD) Write(ctx context.Context, src usermem.IOSequence, opts vfs.WriteOptions) (int64, error) {
	...
	if f.inode.mayBlock {
		...
		writer := fd.NewReadWriter(f.inode.hostFD.FD())
	...
	f.mu.Lock()
	n, err := writeToHostFD(ctx, f.inode.hostFD.FD(), src, f.offset, flags)
	f.mu.Unlock()
	return n, err
}
...
func (f *defaultFileFD) seek(offset int64, whence int32) error {
	...
	case linux.SEEK_END:
		...
		if err := syscall.Fstat(f.inode.hostFD.FD(), &s); err != nil {
			return err
		}
		size := s.Size
	...
	case linux.SEEK_DATA, linux.SEEK_HOLE:
		...
		n, err := unix.Seek(f.inode.hostFD.FD(), offset, int(whence))
		if err != nil {
			return err
		}
		f.offset = n
...
func (f *defaultFileFD) Sync(context.Context) error {
	...
	return unix.Fsync(f.inode.hostFD.FD())
}
